...
<?php
  ...
  $result = mysqli_query($conn,"Select * from Project WHERE judge_id='".$_SESSION['id']."' and evaluated='0'");
echo'  <option value="" selected disabled hidden>Choose here</option>';
  while($row = $result->fetch_assoc()) {

 echo '<option name="search" value="'.$row['project_id'].'">'.$row['project_name'].'</option>';
  }
 echo "</select><br><br>";
...
$result = mysqli_query($conn,"Select * from Project where project_id=".$valor);
//echo $valor;
...
if ($result->num_rows > 0) {
    ...
    echo '<input type="text" name="id_demo" hidden value="'.$row{'demo_id'}.'">';
...
<td>Demo Implementation
<div class="slidecontainer">
 <input name="implementation" type="range" min="1" max="100"value="50" class="slider" id="myRange">
</div>
